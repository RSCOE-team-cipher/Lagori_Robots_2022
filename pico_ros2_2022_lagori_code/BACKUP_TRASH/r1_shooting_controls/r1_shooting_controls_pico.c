...
/* DEFINE */
#define PWM_FEEDER_LEFT 1
#define PWM_FEEDER_RIGHT 2

#define PWM_ANGLE_LEFT 3
#define PWM_ANGLE_RIGHT 4
...
void r1_controls_callback(const void * msgin){
    ...
    (msg->rotor == 1) ? rotors_on() : rotors_off();
    (msg->feeder_up == 1) ? feeder_up() : idle();
    (msg->feeder_down == 1) ? feeder_down() : idle();
    (msg->angle_down == 1) ? angle_down() : idle();
    (msg->angle_up == 1) ? angle_up() : idle();
    (msg->servo == 1) ? servo() : idle();
    (msg->shoot == 1) ? shoot() : idle();
}
