...
/* ROBOT PARAMS */
float l = 0.3; // distance between differential wheels in m
float r = 0.1; // radius of wheels
...
void velocityCb(const void * msgin){
    ...
    float u1 = (-z * l - 2 * x) / (2 * r);
    float u2 = ( z * l - 2 * x) / (2 * r);
    ...
    pwm_write(PWM_1, u1Pwm);
    gpio_put(IN_2, ((u2>0) ? 1 : 0)); 
    pwm_write(PWM_2, u2Pwm);
    sleep_ms(100);
    led(0);
...
int main() {
    ...
    uint slice_num_2 = pwm_gpio_to_slice_num(PWM_2);
