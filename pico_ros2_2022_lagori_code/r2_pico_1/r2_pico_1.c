...
void gripper_one(int one_open, int one_close){
    if(== 1){
        if(gpio_get(gripone_openper1_open_limit) !=0){ //close pressed (closed to max possible)
            led(1);
            gpio_put(G_1_OPEN, 1);
...
void gripper_two(int two_open, int two_close){
    if(two_open == 1){
        if(gpio_get(gripper2_open_limit) !=0){ // if open pressed (closed to max possible)
            led(1);
            gpio_put(G_2_OPEN, 1);
        ...
        if(gpio_get(gripper2_close_limit) !=0){ // if close pressed (closed to max possible)
            led(1);
            gpio_put(G_2_OPEN, 0);
...
void gripperCb(const void * msgin){
    ...
    gripper_one(msg->one_open, msg->one_close);

    //sleep_ms(100);
    multicore_fifo_push_blocking(msg->two_open*100 + msg->two_close*10 + 1);
    /*
      since at a time only 1 integer can pe pushed in buffer
      1, 1 -> 100 + 10 + 1 = 111
      1, 0 -> 100 + 0  + 1 = 101
      0, 1 ->  0  + 10 + 1 = 11
      0, 0 ->  0  + 0  + 1 = 1
...
void core1_interrupt_handler(){
    while(multicore_fifo_rvalid()){
        ...
        gripper_two(two_open, two_close);
    }
...
int main() {
    ...
    while (true)
    {
        ...
        rclc_executor_spin_some(&executor, RCL_S_TO_NS(10)); 
